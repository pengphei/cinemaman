...
class PlayDialog():
    ...
    def _ui_init_addedit(self, title, dtype, wmain, play=None):
        ...
        # hall id combo box
        self.hallidFrame = ttk.LabelFrame(self.top)
        self.hallidFrame.pack(side=tk.TOP)
        self.hallidLabel = ttk.Label(self.hallidFrame, text=STRS_PLAY_LABEL[0], width=8)
        self.hallidLabel.pack(side=tk.LEFT)
        self.hallidCombo = ttk.Combobox(self.hallidFrame)
        self.hallidCombo.config(state="readonly")
        self.hallidCombo['values'] = gInfo.hall_names_list
        self.hallidCombo.pack(side=tk.LEFT)

        # movie id combo box
        self.movieFrame = ttk.LabelFrame(self.top)
        ...
        self.stimeFrame = ttk.LabelFrame(self.top)
        ...
        self.etimeFrame = ttk.LabelFrame(self.top)
        ...
        self.rtimeFrame = ttk.LabelFrame(self.top)
        ...
        self.priceFrame = ttk.LabelFrame(self.top)
    ...
    def add_confirm(self):
        hall_idx = self.hallidCombo.current()
        movie_idx = self.movieidCombo.current()
        play = CMMoviePlay()
        play.id = -1
        # hall referring vars
        ...
        # time referring vars
        play.startTime.year = gInfo.date_focus.year
        play.startTime.month = gInfo.date_focus.month
        play.startTime.day = gInfo.date_focus.day
        self.wmain.play_add_confirm(hall)
        self.top.destroy()
        return
    ...
    def edit_confirm(self):
        ...
        self.wmain.play_edit_confirm(hall)
        self.top.destroy()
        return
